...
def make_windows(out_bed, resolution, n_bins, stride=None, flank=None, chromsizes=DM6):
    C = n_bins * resolution
    F = (BORZOI_INPUT - C) // 2 if flank is None else int(flank)
    stride = stride or (C // 4)

    if flank is None:                       # Borzoi geometry asserts
        assert C <= MAX_SPAN, f"C = {C} exceeds the Borzoi embedding span {MAX_SPAN}"
        assert C % 64 == 0, f"C = {C} must be a multiple of 64"
        assert (F - EMB_HEAD_OFF) % EMB_BIN == 0, f"flank {F} is not on an embedding-bin edge"
    else:                                   # local trunk: only the 2 bp grid matters
        assert resolution % 2 == 0 and F % 2 == 0
    assert stride % resolution == 0
...
if __name__ == "__main__":
    ...
    a = p.parse_args()
    make_windows(a.out, a.resolution, a.n_bins, a.stride, a.flank)